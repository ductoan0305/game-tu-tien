...
export function renderHeader(G) {
  ...
  // Timers / buffs
  renderActiveBuffs(G);
  renderTutorialObjectivePanel(G);
}
...
export function renderTutorialObjectivePanel(G) {
  ...
  const textMap = {
    ...
    2: qi < maxQi
      ...
      : (purity / Math.max(1, threshold)) < 0.5
        ? `Linh lực đã đầy, còn thiếu ~${purityNeedPct}% ngưỡng Thuần Độ tối thiểu để thử đột phá.`
        : 'Đã đủ điều kiện thử đột phá 1 lần (thành/bại đều hợp lệ).',
    3: 'Mở Pháp Địa để xem công pháp đang tu và hướng tăng trưởng.',
    4: 'Mở Nhiệm Vụ để lấy mục tiêu cụ thể thay vì chơi ngẫu hứng.',
  ...
  const rootHint = rootType === 'NGU'
    ...
    : rootType === 'TU'
      ? 'Tứ linh căn: đi đúng nhịp, ưu tiên cửa sổ tuổi.'
      : rootType === 'TAM'
        ? 'Tam linh căn: có cơ hội nhỏ Trúc Cơ nếu kỷ luật.'
        : (rootType === 'SONG' || rootType === 'BIEN_DI' || rootType === 'TIEN')
...
export function showTutorialAgeWarningModal(onAcknowledge) {
  const old = document.getElementById('tutorial-age-warning-modal');
  if (old) return;

  const modal = document.createElement('div');
  modal.id = 'tutorial-age-warning-modal';
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.82);z-index:99999;display:flex;align-items:center;justify-content:center';
  modal.innerHTML = `
    <div style="max-width:420px;width:92%;background:#0e1626;border:1px solid #3a4f72;border-radius:12px;padding:16px 18px;color:#e7eefc">
      <div style="font-size:18px;margin-bottom:8px">⏳ Cảnh Báo Tuổi Tu Luyện</div>
      <div style="font-size:13px;line-height:1.7;color:#c6d4ee">
        Trước 70 tuổi là cửa sổ vàng của Luyện Khí.<br/>
        Từ 70-75, tỷ lệ đột phá giảm rất mạnh.<br/>
        Qua 75 tuổi, gần như vô vọng nếu không có cơ duyên nghịch thiên.
      </div>
      <button id="btn-tutorial-age-ack" style="margin-top:14px;width:100%;padding:10px;background:#5f86c7;color:#fff;border:none;border-radius:8px;cursor:pointer">Đã hiểu</button>
    </div>`;
  document.body.appendChild(modal);

  document.getElementById('btn-tutorial-age-ack')?.addEventListener('click', () => {
    modal.remove();
    onAcknowledge?.();
  });
}