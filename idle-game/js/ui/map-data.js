...
export function svgWorldNodeName(name, x, yBase, opts = {}) {
  ...
  const n = String(name).trim();
  if (n.length <= maxOneLine) {
    return `<text x="${x}" y="${yBase}" text-anchor="middle" font-size="${fs}" fill="${fill}" font-weight="${weight}">${_escapeSvgText(n)}</text>`;
  }
  const mid = Math.floor(n.length / 2);
  let split = n.lastIndexOf(' ', mid + 2);
  if (split <= 0) split = mid;
  ...
  const l2 = n.slice(split).trim();
  return `
    <text x="${x}" y="${yBase - 5}" text-anchor="middle" font-size="${fs - 0.5}" fill="${fill}" font-weight="${weight}">${_escapeSvgText(l1)}</text>
    <text x="${x}" y="${yBase + 6}" text-anchor="middle" font-size="${fs - 0.5}" fill="${fill}" font-weight="${weight}">${_escapeSvgText(l2)}</text>`;
}