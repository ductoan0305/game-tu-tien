...
**Cập nhật lần cuối:** Sprint 10 — LK Content Pass (NPC quests, cơ duyên, age-reactive dialogs) (2026-05-15)
...
| Doc | Khi nào đọc |
|---|---|
| `docs/WORLD_MAP_DESIGN.md` | Bất cứ task gì liên quan map (Tầng 1/2/3, NHAN_GIOI_REGIONS, KHUYETVUC_TERRITORIES, TERRITORY_INTERIORS, starter villages, Painted Scroll style). **HANDOFF KHÔNG còn là source of truth cho map design.** |
...
**Visual direction (quyết định Session L7+):** Painted Scroll Map — parchment background (feTurbulence), terrain illustrations, con dấu chữ Hán theo phe, cloud border, dotted trade routes. **Không dùng PNG** — thuần SVG với reusable symbols. **Implement: 🔲 chưa làm (cả 3 tầng).**
...
**Starter villages:** 50–100+ làng rải khắp Khuyết Vực, phân ngẫu nhiên, **không buff cơ học**. Chỉ là narrative starting point. Render bởi `starter-village.js` khi `!G.worldMap.leftStarter`. KHÔNG render trong tier 2 territory map (Bug MAP-1 đã ghi nhận trong WORLD_MAP_DESIGN.md, chưa fix).
