...
export function doArray(G) {
  ...
  // V1: Không cộng chỉ số vĩnh viễn. Đổi sang buff tốc tu có thời hạn.
  const pct = Math.max(1, Math.round(6 * bonus)); // base +6% (scale theo arrayBonus)
  const dur = 300; // giây thực
  G.eventRateBonus = Math.max(G.eventRateBonus || 0, pct);
  G.eventRateTimer = Math.max(G.eventRateTimer || 0, dur);
  gainExp(G, 20);

  return {
    ok:true,
    msg:`🔮 Bố ${name} — tốc tu +${pct}% trong ${Math.floor(dur/60)} phút`,
    type:'spirit',
    float:`+${pct}%`,
  };
}
