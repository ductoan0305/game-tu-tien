...
// Căn Cốt thực = G.canCot + bonus từ Tẩy Tủy Quyết (canCotBonus)
// Dùng hàm này ở mọi nơi cần đọc canCot, không đọc G.canCot trực tiếp
export function calcEffectiveCanCot(G) {
  const base     = G.canCot ?? 50;
  const cpBonus  = calcCongPhapMasteryBonus(G).canCotBonus ?? 0;
  return Math.min(100, base + cpBonus);
}
