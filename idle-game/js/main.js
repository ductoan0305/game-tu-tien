...
import { renderHeader, renderCultivateStats, renderCombatStatus,
         ...
         checkAndNotifyUnlocks, toggleNotifPanel,
         showTutorialAgeWarningModal }                                from './ui/render-core.js';
...
import { renderWorldMap, updateMapStats, openWorldMapModal,
...
import { ensureTutorialState, updateTutorialStep,
         trackMeditateSeconds, trackStaminaAction,
...
function tick() {
  ...
  try {
    gameTick(G, 0.1);
    if (G.meditating) trackMeditateSeconds(G, 0.1);
    updateTutorialStep(G);
    if (G.tutorial?.enabled && G.tutorial?.step === 5 && !G.tutorial.seenHints?.age_warning_modal) {
      G.tutorial.seenHints.age_warning_modal = true;
      showTutorialAgeWarningModal(() => {
        acknowledgeAgeWarning(G);
        updateTutorialStep(G);
        renderAll();
      });
    }
...
const mapActions = {
  ...
  explore: () => {
    ...
    if (result?.ok) trackStaminaAction(G);
    handleAction(result);
  ...
  forge: (recipeId) => {
    import('./alchemy/crafting-data.js').then(({ CRAFTING_RECIPES, getCraftsmanRank, FORGE_SUCCESS_BONUS, CRAFTABLE_EQUIPMENT }) => {
      import('./equipment/equipment-data.js').then(({ EQUIPMENT }) => {
        ...
        if ((G.realmIdx||0)<(recipe.requireRealm||0)) { showToast(`🔒 Cần đạt ${REALM_NAMES[recipe.requireRealm]} trước!`,'danger'); return; }
...
function wireEvents() {
  ...
  document.getElementById('bnav-btn-world-map')?.addEventListener('click', (e) => {
    ...
    e.stopPropagation();
    if (!G) return;
    openWorldMapModal(G, mapActions);
  });
